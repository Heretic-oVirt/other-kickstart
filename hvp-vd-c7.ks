...
# elevator=deadline ip=nicname:dhcp inst.ks=https://dangerous.ovirt.life/hvp-repos/el7/ks/hvp-vd-c7.ks
...
if [ -f "/sys/class/net/${ks_nic}/address" ]; then
	ks_custom_frags="${ks_custom_frags} hvp_parameters_vd.sh hvp_parameters_$(cat /sys/class/net/${ks_nic}/address).sh"
else
	ks_custom_frags="${ks_custom_frags} hvp_parameters_vd.sh"
fi
if [ -z "${ks_source}" ]; then

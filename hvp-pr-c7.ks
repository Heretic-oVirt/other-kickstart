...
# nomodeset elevator=deadline ip=nicname:dhcp inst.ks=https://dangerous.ovirt.life/hvp-repos/el7/ks/hvp-pr-c7.ks
...
if [ -f "/sys/class/net/${ks_nic}/address" ]; then
	ks_custom_frags="${ks_custom_frags} hvp_parameters_pr.sh hvp_parameters_$(cat /sys/class/net/${ks_nic}/address).sh"
else
	ks_custom_frags="${ks_custom_frags} hvp_parameters_pr.sh"
fi
if [ -z "${ks_source}" ]; then
...
if [ -s /tmp/hvp-pr-conf/rc.pr-provision ]; then
	cp /tmp/hvp-pr-conf/rc.pr-provision /mnt/sysimage/etc/rc.d/rc.pr-provision
	# Note: cleartext passwords contained - must restrict access
	chmod 700 /mnt/sysimage/etc/rc.d/rc.pr-provision
	chown root:root /mnt/sysimage/etc/rc.d/rc.pr-provision
fi

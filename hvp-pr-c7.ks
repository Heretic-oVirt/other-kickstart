...
volgroup PRVG pv.01
# Define swap space
logvol swap --vgname=PRVG --name=swap --fstype=swap --recommended
logvol / --vgname=PRVG --name=root --size=6000
logvol /var --vgname=PRVG --name=var --size=2000
logvol /var/cache --vgname=PRVG --name=var_cache --size=5000
logvol /var/crash --vgname=PRVG --name=var_crash --size=12000
logvol /var/lib --vgname=PRVG --name=var_lib --size=10000
logvol /var/log --vgname=PRVG --name=var_log --size=10000
logvol /var/log/audit --vgname=PRVG --name=var_log_audit --size=2000
logvol /var/spool --vgname=PRVG --name=var_spool --size=3000 --grow
logvol /var/tmp --vgname=PRVG --name=var_tmp --size=2000
logvol /home --vgname=PRVG --name=home --size=1000
logvol /tmp --vgname=PRVG --name=tmp --size=2000
EOF
...
if [ "${domain_join}" = "true" ]; then
	...
	if [ \${res} -ne 0 ]; then
		...
		# TODO: find a Winbind equivalent for auto_private_groups setting

# Kickstart file for template server
...
# nomodeset elevator=deadline inst.ks=https://dangerous.ovirt.life/hvp-repos/el7/ks/hvp-tmpl-c7.ks
...
# Note: the default machine IPs are assumed to be the 180th IPs available (network address + 180) on each connected network
...
text
# Using poweroff for a template machine
# (must be sure that system does not try to loop-install again and again)
# Note: this is needed for proper installation automation by means of cloud-init
poweroff
...
ks_custom_frags="hvp_parameters.sh hvp_parameters_tmpl.sh"
mkdir /tmp/kscfg-pre
...
# Create a VG
volgroup TmplVG pv.01
# Define swap space
logvol swap --vgname=TmplVG --name=swap --fstype=swap --recommended
logvol / --vgname=TmplVG --name=root --size=6000
logvol /var --vgname=TmplVG --name=var --size=2000
logvol /var/cache --vgname=TmplVG --name=var_cache --size=5000
logvol /var/crash --vgname=TmplVG --name=var_crash --size=12000
logvol /var/lib --vgname=TmplVG --name=var_lib --size=10000 --grow
logvol /var/log --vgname=TmplVG --name=var_log --size=10000
logvol /var/log/audit --vgname=TmplVG --name=var_log_audit --size=2000
logvol /var/spool --vgname=TmplVG --name=var_spool --size=3000
logvol /var/tmp --vgname=TmplVG --name=var_tmp --size=2000
logvol /home --vgname=TmplVG --name=home --size=1000
logvol /tmp --vgname=TmplVG --name=tmp --size=2000
EOF
...
if [ "${domain_join}" = "true" ]; then
	...
	if [ \${res} -ne 0 ]; then
	        ...
	        klist
	        realm join -v --unattended --os-name=\$(lsb_release -si) --os-version=\$(lsb_release -sr) --computer-ou=OU="Application Servers" --automatic-id-mapping=no ${ad_subdomain_prefix}.${domain_name[${my_zone}]}
...
mkdir -p /tmp/hvp-tmpl-conf
pushd /tmp/hvp-tmpl-conf
cat << EOF > /tmp/hvp-tmpl-conf/rc.tmpl-provision
#!/bin/bash
exit 0
EOF
...
ks_custom_frags="hvp_parameters.sh hvp_parameters_tmpl.sh hvp_parameters_*:*.sh"
...
# Install needed packages to configure the template on initial run
# TODO:  cloud-utils-growpart does not support LVM - add back when feature added upstream
...
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">
	...
	<body>
	<h1><strong>Template server</strong></h1>

		<div class="content">
			<div class="content-columns">
				<div class="content-column-left">
					<h2>Avvertenza per gli utenti del servizio:</h2>
					<p>Questa macchina &egrave; un template generico.</p>
				...
				<div class="content-column-right">
					...
					<p>This machine is a generic template.</p>
...
#!/bin/bash
# Nothing to do for a template server
exit 0
EOF
...
# Run template provisioning actions
if [ -x /etc/rc.d/rc.tmpl-provision ]; then
	/etc/rc.d/rc.tmpl-provision
fi
...
# Copy template configuration script (generated in pre section above) into installed system
if [ -s /tmp/hvp-tmpl-conf/rc.tmpl-provision ]; then
	cp /tmp/hvp-tmpl-conf/rc.tmpl-provision ${ANA_INSTALL_PATH}/etc/rc.d/rc.tmpl-provision
	# Note: cleartext passwords contained - must restrict access
	chmod 700 ${ANA_INSTALL_PATH}/etc/rc.d/rc.tmpl-provision
	chown root:root ${ANA_INSTALL_PATH}/etc/rc.d/rc.tmpl-provision
fi
